...
from tools.audio_generation.openai import OpenAIAudioGenerator
from tools.common.base_model import BaseModelTool
from tools.common.messenger import Messenger
from tools.image_generation.gemini import GeminiImageGenerator
from tools.image_generation.openai import OpenAIImageGenerator
from tools.image_generation.pollinations import PollinationsImageGenerator
from tools.image_generation.replicate_gen import ReplicateImageGenerator
...
from tools.text_generation.groq import GroqTextGenerator
...
class Pipeline(BaseModelTool):
    ...
    _text_gen: Optional[Union[OpenAITextGenerator, GeminiTextGenerator, GroqTextGenerator]] = PrivateAttr(default=None)
    _image_gen: Optional[Union[OpenAIImageGenerator, GeminiImageGenerator, PollinationsImageGenerator, ReplicateImageGenerator]] = PrivateAttr(default=None)
    _audio_gen: Optional[Union[OpenAIAudioGenerator, GeminiAudioGenerator]] = PrivateAttr(default=None)
    ...
    @property
    def text_gen(self) -> Union[OpenAITextGenerator, GeminiTextGenerator, GroqTextGenerator]:
        if self._text_gen is None:
            provider = os.getenv("AI_PROVIDER", "gemini").lower()
            if provider == "openai":
                self._text_gen = OpenAITextGenerator()
            elif provider == "groq":
                self._text_gen = GroqTextGenerator()
            else:
                self._text_gen = GeminiTextGenerator()
        return self._text_gen

    @property
    def image_gen(self) -> Union[OpenAIImageGenerator, GeminiImageGenerator, PollinationsImageGenerator, ReplicateImageGenerator]:
        if self._image_gen is None:
            ar_value = "9:16" if self.orientation == VideoOrientation.SHORT else "16:9"
            
            # Check for specific image provider, otherwise fallback to AI_PROVIDER
            provider = os.getenv("IMAGE_PROVIDER", os.getenv("AI_PROVIDER", "gemini")).lower()
            
            if provider == "openai":
                self._image_gen = OpenAIImageGenerator(aspect_ratio=ar_value)
            elif provider == "pollinations":
                self._image_gen = PollinationsImageGenerator(aspect_ratio=ar_value)
            elif provider == "replicate":
                self._image_gen = ReplicateImageGenerator(aspect_ratio=ar_value)
            else:
    ...
    @property
    def audio_gen(self) -> Union[OpenAIAudioGenerator, GeminiAudioGenerator]:
        if self._audio_gen is None:
            provider = os.getenv("AI_PROVIDER", "gemini").lower()
            if provider == "openai":
                self._audio_gen = OpenAIAudioGenerator()
            else:
    ...
    def step1_generate_story(self):
        ...
        1. Checks for PENDING ideas first.
        2. Generates concept and script using PromptManager.
        3. Registers/Updates the idea in tracking CSV.
        4. Saves idea.json and script.json.
        5. Updates state to SCRIPT_GENERATED.
        """
        Messenger.info("\n--- Generating cinematic concept and script ---")

        # Check active work limit before starting new ideas
        df = self.store.read_all()
        active_states = [
            State.SCRIPT_GENERATED.value,
            State.IMAGES_GENERATED.value,
            State.AUDIO_GENERATED.value,
            State.VIDEO_GENERATED.value,
            State.VIDEO_SUBTITLED.value,
            State.VIDEO_MUSIC_GENERATED.value
        ]
        active_count = df[df['state'].isin(active_states)].shape[0]

        if active_count >= 2:
            Messenger.info(f"Skipping step 1: Max active ideas reached ({active_count}/2). Finishing backlog first.")
            return

        # 1. Check for PENDING ideas
        pending_idea = self.store.get_first_by_state(State.PENDING)

        if pending_idea:
            Messenger.info(f"Fulfilling PENDING idea: {pending_idea.title}")
            idea_data, script = self.prompt_manager.generate_story_from_idea(
                self.text_gen, pending_idea.title, pending_idea.category
            )
            idea_obj = pending_idea
        else:
            # 2. Random Selection (No pending work at all)
            Messenger.info("No pending work found. Generating a new random idea...")
    ...
    def step2_generate_images(self):
        ...
        # 3. Solo generamos la imagen para la escena 1 (Formato Static Avatar)
        all_tasks: List[ImageTask] = []
        out_path = self.get_idea_asset_path(
            idea_obj.id, self.IMAGES_DIR, self.SCENE_IMAGE_PATTERN.format(1)
        )
        if out_path.exists():
            Messenger.info(f"Skipping Scene 1 image: File already exists.")
        else:
            all_tasks.append(
                ImageTask(prompt=script_data.scenes[0].image_prompt.formatted_prompt, output_path=out_path)
            )
    ...
    def step3_generate_audios(self):
        ...
        for start_idx in range(0, total_scenes, batch_size):
            ...
            for al in alignment.alignments:
                ...
                # Add a small buffer (0.5s) ONLY to the last scene of the entire script to prevent cuts at the end
                is_last_scene = (absolute_scene_num == total_scenes)
    ...
    def step4_generate_videos(self):
        ...
        for i in range(len(script_data.scenes)):
            # Formato Static Avatar: Siempre usamos la imagen de la escena 1
    ...
    def step5_generate_subtitles(self):
        ...
        # 4.5 Auto-correct common DJ terminology in subtitles
        if subs_srt.exists():
            srt_text = subs_srt.read_text(encoding="utf-8")
            
            # Reverb corrections
            srt_text = re.sub(r'(?i)riverb', 'Reverb', srt_text)
            srt_text = re.sub(r'(?i)river', 'Reverb', srt_text)
            
            # Echo corrections
            srt_text = re.sub(r'(?i)\beco\b', 'Echo', srt_text)
            
            # HPF/LPF corrections
            srt_text = srt_text.replace("HPF", "High Pass Filter").replace("LPF", "Low Pass Filter")
            
            subs_srt.write_text(srt_text, encoding="utf-8")
    ...
    def step7_rename_final_video(self):
        ...
        final_video.replace(named_final)
