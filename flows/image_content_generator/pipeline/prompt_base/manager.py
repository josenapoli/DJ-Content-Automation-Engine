...
class BasePromptManager(BaseModel):
    ...
    def get_config_by_name(self, category_name: str) -> SelectedConfig:
        """Finds a specific configuration based on a combined category name."""
        for handler in self.CATEGORIES:
            for idea_model in handler.idea_variants:
                if f"{handler.__name__}_{idea_model.__name__}" == category_name:
                    return SelectedConfig(
        ...
        raise ValueError(f"Category configuration not found: {category_name}")
